...
def evaluate_query(example: Dict[str, Any], model: str, include_samples: bool, few_shot_examples: List[Dict[str, Any]] = None, use_mcts: bool = True, mcts_iterations: int = 50) -> Dict[str, Any]:
    ...
    try:
        ...
        if use_mcts:
            ...
            if test_db_dir.exists():
                ...
                rarp = MCTSRARP(db_id, model, few_shot_examples=few_shot_examples, mcts_iterations=mcts_iterations)
        else:
...
def evaluate_dataset(dataset_file: str, model: str, 
                    ...
                    num_few_shot: int = 0,
                    use_mcts: bool = True,
                    mcts_iterations: int = 50) -> Dict[str, Any]:
    ...
    if num_processes > 1:
        with multiprocessing.Pool(processes=num_processes) as pool:
            results = list(tqdm(
                pool.starmap(
                    evaluate_query, 
                    [(example, model, include_samples, few_shot_examples, use_mcts, mcts_iterations) for example in examples]
                ),
                total=len(examples)
            ))
    else:
        ...
        for example in tqdm(examples, desc="Evaluating"):
            result = evaluate_query(example, model, include_samples, few_shot_examples, use_mcts, mcts_iterations)
...
def main():
    ...
    parser.add_argument("--mcts-iterations", type=int, default=50, help="Number of MCTS iterations")
